...
class ConsentBanner {
  ...
  init() {
    ...
    if (consent) { this.initialized = true; return; }
    this.render();
  ...
  render() {
    ...
    this.banner.addEventListener('keydown', (e) => { if (e.key === 'Escape') this.decline(); });
    requestAnimationFrame(() => this.banner.classList.add('visible'));
    setTimeout(() => document.getElementById('consent-accept')?.focus(), 100);
  }

  accept() {
    ga4.updateConsent(true);
    this.hide();
    ga4.track('consent_granted', { consent_type: 'analytics' });
  }
  ...
  hide() {
    ...
    setTimeout(() => { this.banner.remove(); this.banner = null; }, 300);
  }

  showSettings() {
    window.dispatchEvent(new CustomEvent('open-privacy-settings'));
    if (!document.querySelector('.privacy-settings-modal')) this.showSimpleSettings();
  }
  ...
  showSimpleSettings() {
    ...
    modal.innerHTML = `
      <div class="privacy-settings-content">
        ...
        <div class="privacy-option">
          <label><input type="checkbox" id="analytics-toggle" checked><span>Analytics Cookies</span></label>
    ...
    document.getElementById('privacy-cancel').addEventListener('click', () => modal.remove());
